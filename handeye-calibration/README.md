...
```bash
# 启动 ROS2 相机节点（确保相机已连接）
# 然后运行采集脚本：
source /opt/ros/humble/setup.bash
python3 online_capture.py

# 键位：c/空格=采集，q/ESC=退出
# 可选参数（覆盖 config.yaml）：
python3 online_capture.py --ws-host 192.168.x.x --ws-port 8765
```

### 3. 计算手眼外参

```bash
python3 eyeInHand.py
...
```bash
python3 remove_image_samples.py --remove 0,2,5
# 自动重新编号剩余样本
```
