...
└── pose/
    ...
    ├── requirements.txt
    ├── yolo11n-pose.pt          ← 预训练基础权重（首次自动下载）
...
python3 pose/src/prepare_data.py

# 模式2：已有 YOLO txt，只做 train/val/test 分割
python3 pose/src/prepare_data.py --mode split-only

# 可选 CLI 覆盖：
python3 pose/src/prepare_data.py --src pose/my_dataset --dst pose/yolo_my --train 0.85 --val 0.1
```

### 4. 训练

```bash
python3 pose/src/train.py
...
```bash
python3 pose/src/realtime_detect_pose.py
# 使用 config.yaml 中的 inference.weights_path 作为默认权重
# 或指定：
python3 pose/src/realtime_detect_pose.py --weights pose/runs/my_experiment/weights/best.pt
```
